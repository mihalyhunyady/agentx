...
import com.epam.agentx.game.service.LobbyService
...
@RestController
class GameController @Autowired constructor(val userRepository: UserRepository, val roomRepository: RoomRepository, val template: SimpMessagingTemplate, val lobbyService: LobbyService, val gameService: GameService) {
    ...
    fun joinRoom(@PathVariable("id") roomId: Long, @RequestBody event: GameEvent): Response {
        ...
        if (event.event.equals(Event.JOIN)) {
            lobbyService.joinGame(user, { players ->
            ...
            lobbyService.leaveGame(user, { players ->
